...
plot $data0 using 1:2 with lines title sprintf('PDF (α = %4.2f)', α),\
     $data0 using 1:2 with filledcurves y=0 fill solid 0.125 noborder notitle
"

let renderShape path pdf α =
...
if (style eq 'C') {{ set linetype 2 linewidth 2 linecolor rgb baseBlue }}
if (style eq 'S') {{ set linetype 2 linewidth 2 linecolor rgb baseMint }}
