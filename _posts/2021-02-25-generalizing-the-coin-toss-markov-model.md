...
{% latex fig-20 %}
    ...
    \begin{displaymath}
    \begin{aligned}
    & \mathrlap{R_{\mathrm{even}}}\phantom{R_{\mathrm{even}}}
    =
    \{\ 0, 2, 4, \dots, n \ \}
    \\
    & \mathrlap{R_{\mathrm{odd}}}\phantom{R_{\mathrm{even}}}
    =
    \{\ 1, 3, 5, \dots, n \ \}
    \end{aligned}
    \end{displaymath}
...
{% latex fig-21 %}
    ...
    R =
    \begin{dcases}
    R_{\mathrm{even}} & \quad \text{if $n$ is even}
    \\
    R_{\mathrm{odd}}  & \quad \text{if $n$ is odd}
    \end{dcases}
    \end{displaymath}
    \end{document}
{% endlatex %}
...
Another problem is runtime performance. With the implementation used in this post, finding the solution for a model with ten coin toss events per round takes a couple of seconds to execute on my current hardware. A model with one additional coin toss event per round takes about twice the amount of time to execute. This implementation seems to have quadratic time complexity. There is no parallelism, and I have made no attempt at performance tuning. But I do think there is some room for improvement. There might also be other approaches that give a good enough solution. Ideally, I would like to be able to solve for models with twenty or even fifty flips per round.
